...
var convertFunc = function convertFunc(op, vals, conv, config, not, fieldConfig, meta) {
  ...
  if (funcKeys.length) {
    ...
    for (var _i2 = 0, _Object$entries = Object.entries(config.funcs || {}); _i2 < _Object$entries.length; _i2++) {
...
var convertConj = function convertConj(op, vals, conv, config, not, meta) {
  ...
  if (conjKey) {
    ...
    var complexFieldsConfigs = arrayToObject(arrayUniq(complexFieldsParents).map(function (f) {
      return [f, (0, _configUtils.getFieldConfig)(config, f)];
    }));
    var complexFieldsInRuleGroup = complexFieldsParents.filter(function (f) {
      return complexFieldsConfigs[f].type == "!group";
    });
    var usedRuleGroups = arrayUniq(complexFieldsInRuleGroup);
    ...
    var children1 = {}; // TIP: `needSplit` will be true if using mode=struct and there are fields of different groups on one level
      ...
      if (v.type == "group" || v.type == "rule_group") {
        ...
        if (!groupField) {
          ...
          groupField.split(fieldSeparator).map(function (f, i, a) {
            ...
            ff = (0, _configUtils.normalizeField)(config, ff);
            var ffConfig = (0, _configUtils.getFieldConfig)(config, ff) || {};
...
var _parseRule = function _parseRule(op, arity, vals, parentField, conv, config, errors, isRevArgs) {
  ...
  var isGroup0 = !isAllInForMultiselect && ["some", "all", "none"].includes(op);
...
var convertOp = function convertOp(op, vals, conv, config, not, meta) {
  ...
  var opConfig = config.operators[opKey];
  var canRev = !(fieldConfig.type == "!group");
